...
def main(args):
    ...
    question_embeddings = np.load(args.data_dir + 'questions_atom_aware_bi_' + args.embedder + '.npy')
    ...
    if args.qu_count > 1:
        for count in range(2, args.qu_count+1):
            curr_question_embeddings = np.load(args.data_dir + 'questions_atom_aware_bi_' + str(count) + '_' + args.embedder + '.npy')
