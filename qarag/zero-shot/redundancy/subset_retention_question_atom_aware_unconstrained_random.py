...
def main(args):
    ...
    for count in range(1,MAX+1):
        if count == 1:
            all_question_embeddings.append( np.load(args.data_dir + 'questions_atom_aware_' + args.embedder + '.npy') )
        else:
            all_question_embeddings.append( np.load(args.data_dir + 'questions_atom_aware_' + str(count) + '_' + args.embedder + '.npy') )
    ...
    assert len(atom_idx_to_chunk_idx) == len(all_question_embeddings)

    # restructure
    grouped_atomic_question_embeddings = {}
    for atom_idx, chunk_idx in enumerate(atom_idx_to_chunk_idx):
    ...
    #fracs = [0.095, 0.158, 0.217, 0.317, 0.453, 1.0]
